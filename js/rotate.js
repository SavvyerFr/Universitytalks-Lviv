jQuery(document).ready(function($){
	...
	bindEvents(MQ, true, screenWidth);
	
	$(window).on('resize', function(){
		MQ = deviceType();
		var screenWidth = screen.width;
		bindEvents(MQ, bindToggle, screenWidth);
		//console.log(MQ);
    ...
    function bindEvents(MQ, bool, screen) {
    	
		if(screen > 1050) { $('body').addClass('desktop').removeClass('mobile');
		}else{
    	...
    	if( MQ == 'desktop' && bool && screen > 1050) {   		
	...
	/*$('.vertical-nav li').click(function(e){
		nextSection();
	})*/
	...
	function checkNav(){
		...
		$('section').each(function(){
			...
			if($('#' + id).hasClass('visible')){
				$('.vertical-nav a[href=#' + id + ']').addClass('active');
			}
		})
	}
